...
renderPlotly({
  ...
  sonmza <- readr::read_csv_chunked(paste0("datos/datos urbanos/",input$mun,"_caract.csv"), 
  ...
  loc_aggr <- sonmza %>% 
    group_by(valor) %>% summarise(Manzanas=sum(num)) %>% 
    mutate(pctj=round(Manzanas*100/sum(Manzanas),0),
           Proporción=paste0(pctj,"%")) %>% 
     mutate(clasf_prop=case_when(valor==3~ "Todas las\nvialidades",
                            valor== 2 ~ "Alguna\nvialidad",
                            valor==1 ~ "Ninguna\nvialidad")) %>% 
